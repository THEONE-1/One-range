...
用户                    前端(Vue)              后端(Spring Boot)         数据库(MySQL)
 │                         │                         │                         │
 │  访问 /register         │                         │                         │
 │────────────────────────>│                         │                         │
 │                         │                         │                         │
...
└────────────────────────┬────────────────────────────────────────┘
                         ...
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│              VulnTemplate.vue 组件加载                          │
│              传入 moduleKey="xss_reflect"                       │
└────────────────────────┬────────────────────────────────────────┘
                         │
                         ├─→ 检查模块完成状态
                         │   GET /api/progress/check?vulnModule=xss_reflect
                         │
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│           LearningProgressController.checkCompleted()           │
│                         │                                       │
│                         ▼                                       │
│           LearningProgressService.checkCompleted()              │
│                         │                                       │
│                         ▼                                       │
│           LearningProgressDao → SELECT FROM learning_progress   │
└────────────────────────┬────────────────────────────────────────┘
                         │
                         ▼
              ┌──────────────────────┐
...
│              vulnModule=xss_reflect                             │
└────────────────────────┬────────────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│           LearningProgressController.markCompleted()            │
│                         │                                       │
│                         ▼                                       │
│           LearningProgressService.markCompleted()               │
│                         │                                       │
│                         ▼                                       │
│           LearningProgressDao → INSERT INTO learning_progress   │
│           (user_id, vuln_module, completed_at)                  │
└────────────────────────┬────────────────────────────────────────┘
                         │
                         ▼
              ┌──────────────────────┐
...
│    │                                                            │
│    ├─→ 检查 localStorage.getItem('username')                    │
│    │                                                            │
│    └─→ 未登录 → 重定向到 /login                                 │
│        已登录 → 继续加载组件                                    │
└────────────────────────┬────────────────────────────────────────┘
...
┌─────────────────────────────────────────────────────────────────┐
│【阶段 2：加载漏洞页面组件】                                      │
│  frontend/src/views/vulns/XSSReflect.vue                        │
│    │                                                            │
│    ├─→ 引入 VulnTemplate.vue 模板组件                           │
│    │   传入 moduleKey="xss_reflect"                             │
│    │                                                            │
│    ├─→ VulnTemplate.vue::onMounted()                            │
│    │   调用 checkModuleStatus() 检查完成状态                    │
│    │                                                            │
│    └─→ 显示漏洞说明、测试表单、源码分析                         │
└────────────────────────┬────────────────────────────────────────┘
                         │
...
┌─────────────────────────────────────────────────────────────────┐
│【阶段 3：用户提交测试数据】                                      │
│  XSSReflect.vue::handleSubmit()                                 │
│    │                                                            │
│    ├─→ 获取用户输入 formState.content                           │
│    │                                                            │
│    └─→ axios.get('/home/xss/reflect', { params: { content } })  │
│        发送 HTTP 请求到后端                                     │
└────────────────────────┬────────────────────────────────────────┘
                         │
                         ▼
...
└────────────────────────┬────────────────────────────────────────┘
        ...
        ▼                ▼                ▼                ▼
┌───────────────┐ ┌───────────────┐ ┌───────────────┐ ┌───────────────┐
│  XSS 漏洞     │ │  SQL 注入     │ │  RCE 命令执行 │ │  其他漏洞     │
└───────┬───────┘ └───────┬───────┘ └───────┬───────┘ └───────┬───────┘
        │                 │                 │                 │
        ▼                 ▼                 ▼                 ▼

【XSS 反射型】
controller/basevul/XSSVul.java::xssReflect()
    │
    └─→ return content;  // 直接返回用户输入，无过滤
        漏洞点：未进行 HTML 编码

【SQL 注入 - 整型】
controller/basevul/sqli/JDBC.java::int_based()
    │
...
【RCE - Runtime】
controller/basevul/rce/RuntimeExec.java::RuntimeExec()
    │
...
【XSS 存储型】
controller/basevul/XSSVul.java::xssInsert()
    │
    ├─→ xssService.setContent(new Xss(content))
    │   存储用户输入到数据库
    │
    └─→ xssService.getContent()
        返回所有存储内容，无过滤
...
│    │   使用 v-html 渲染（XSS 漏洞触发点）                       │
│    │                                                            │
│    └─→ watch(result) → nextTick()                               │
│        手动执行 script 标签（演示 XSS 效果）                    │
└────────────────────────┬────────────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│【阶段 6：标记学习完成（可选）】                                  │
│  VulnTemplate.vue::markComplete()                               │
│    │                                                            │
│    ├─→ axios.post('/api/progress/complete', { vulnModule })     │
│    │                                                            │
│    ├─→ LearningProgressController.markCompleted()               │
│    │                                                            │
│    ├─→ LearningProgressService.markCompleted()                  │
...
| 漏洞类型 | 前端组件 | 后端 Controller | API 路径 |
|---------|---------|----------------|----------|
| XSS 反射型 | XSSReflect.vue | XSSVul.java | GET /home/xss/reflect |
| XSS 存储型 | XSSStore.vue | XSSVul.java | GET /home/xss/store |
| SQL 整型注入 | SQLIntBased.vue | JDBC.java | GET /home/sqli/jdbc/int_based |
| SQL 报错注入 | SQLErrorBased.vue | JDBC.java | GET /home/sqli/jdbc/error_based |
| SQL 时间盲注 | SQLTimeBased.vue | JDBC.java | GET /home/sqli/jdbc/blind_time_based |
| SQL 布尔盲注 | SQLBoolBased.vue | JDBC.java | GET /home/sqli/jdbc/blind_bool_based |
| RCE Runtime | RCERuntime.vue | RuntimeExec.java | GET /home/rce/runtime |
| RCE ProcessBuilder | RCEProcessBuilder.vue | ProcessBuilderExec.java | GET /home/rce/processbuilder |
| 文件上传 | FileUpload.vue | UploadVul.java | POST /home/file/upload |
| 文件下载 | FileDownload.vue | DownloadVul.java | GET /home/file/download |
| SSRF | SSRF.vue | SSRFVul.java | GET /home/ssrf |
| XXE | XXEXMLReader.vue | XXE.java | POST /home/xxe/* |
